...
export const BookmarkCard = ({
  ...
  return (
    <button
      ...
      className="group overflow-hidden relative text-left w-full cursor-pointer bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-2xl hover:border-blue-300/60 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 hover:-translate-y-1"
    >
      <Button
        variant="ghost"
        size="sm"
        onClick={(e) => onToggleFavorite(url.id, url.is_favorite, e)}
        className="absolute top-4 left-4 z-10 p-2 bg-white/90 hover:bg-white border border-slate-200/60 rounded-xl shadow-lg shadow-slate-500/10 hover:shadow-slate-500/20 transition-all duration-200 backdrop-blur-sm"
      ...
      <nav className="absolute top-4 right-4 menu-container transition-all duration-200">
        ...
        {openMenuId === url.id && (
          <Card
            variant="elevated"
            padding="none"
            className="absolute right-0 mt-2 z-10 min-w-[120px] overflow-hidden"
          >
            <Button
              variant="ghost"
              size="sm"
              onClick={(e) => onDelete(url.id, e)}
              className="block w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-red-50/80 transition-colors font-medium rounded-none"
            >
              削除
            </Button>
          </Card>
      ...
      <Button
        ...
        className="absolute bottom-0 right-0 p-4 bg-gradient-to-r from-blue-500/10 to-indigo-500/10 hover:from-blue-500/20 hover:to-indigo-500/20 border-t border-l border-blue-200/50 rounded-tl-xl transition-all duration-200 opacity-0 group-hover:opacity-100 backdrop-blur-sm"
        title="新しいタブで開く"
      >
