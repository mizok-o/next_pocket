...
export default function BookmarkList() {
  ...
  const handleImageError = (urlId: number) => {
    setImageErrors(prev => new Set(prev).add(urlId));
  };
    ...
    <ul className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      {urls.map((url) => (
        <li key={url.id} className="contents">
          ...
          >
            <figure className="aspect-w-16 aspect-h-9 relative h-36 bg-gradient-to-br from-slate-50 to-blue-50/30 border-b border-slate-100/80 overflow-hidden">
